...
fs::file_copy(
  fs::path(series_path, "output", "01", keep),
  fs::path(series_path, "series-fake", "image", keep)
)
keep <- c("art-system_02_0003.png", "art-system_02_0005.png")
fs::file_copy(
  fs::path(series_path, "output", "02", keep),
  fs::path(series_path, "series-fake", "image", keep)
)
fs::dir_tree(
  series_path, 
...
create_resized_images(
  series = "series-fake",
  images_from = "series-fake/image",
  images_to = "series-fake/preview",
  pixels_wide = 500,
  pixels_high = 500,
  origin = tempdir()
)
...
```{r}
#| echo: false
fs::dir_tree(series_path, all = TRUE, recurse = TRUE)
```

The repository now contains the "full resolution" images in the `series-fake/image` folder, and lower resolution versions of the same images in the `series-fake/preview` folder. Or, to narrow things down to the folder that we're planning to publish, this is where we're at:

```{r}
#| echo: false
publish_dir <- fs::path(series_path, "series-fake")
fs::dir_tree(publish_dir, all = TRUE, recurse = TRUE)
```
...
The `series-fake` folder (... contained within the `series-fake` repository, which should kinda be a hint that it's going to be moved later!) isn't really intended to stay in the repository forever. Rather, it's something that we're going to publish to the web eventually. It's going to live in the cloud storage bucket listed in `bucket_remote_path()`. 
However, when we move it online, we're going to need a manifest file that lists all the files contained in the `series-fake` bucket. We need this so that the website that we're going to build later on will have an easy way to discover all the images in our curated set. 

Yes, this is getting exhausting. Fortunately, the arttools package has a function called `manifest_write()` that will create exactly the file we need:

```{r}
# yes, this is something that I will need to tidy later:
# I wrote manifest_write() thinking it would be called 
# with respect to the local bucket path not the local 
# repo path, and its not robust if you do it the other 
# way around. So this very ugly note is here to remind
# me that this part of the package/workflow needs to be
# reconsidered
blah <- file.path(tempdir(), "series-fake")

manifest_write("series-fake", origin = blah, destination = blah)
```
...
```{r}
manifest_read("series-fake", origin = blah)
```

At this point we have a complete folder that we can drop straight into the remote bucket and publish.
